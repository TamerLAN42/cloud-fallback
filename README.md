...
- 2 Linux-машины с доступом в интернет;
- Установленные `git`, `docker` и `docker-compose`;
- Аккаунт в Yandex cloud с созданным сервисным аккаунтом;
- ssh-клиент для генерации ключей.
...
graph TB
    subgraph "Нормальная работа"
        A[Основной хост] -->|бекапы| B[Бекап-хост]
        B -->|мониторинг| A
    end
    
    subgraph "Аварийный сценарий"
        B -->|падение| C[Создан LOCK_STATE]
        C -->|запускает| D[Terraform]
        D -->|создаёт и настраивает| E[ВМ в облаке]
    end
```
...
А я только намекну - нам нужен .json файл с авторизованным ключом от сервисного аккаунта с правами роли `editor` и выше.  
...
Либо:
- `CHECK_URL` - Полный url сервиса для мониторинга, включая протокол и порт. Например `https://example.orp:123`. Если `CHECK_URL` указан - `CHECK_HOST` и `CHECK_PORT` игнорируются.

6. После того как обязательные переменные установлены, можно приступать к запуску.  
Обратите внимание, у компонентов зависимость друг от друга, поэтому запустить их надо +- одновременно. Речь идёт о интервале менее чем 10-30 секунд. Будет хорошо, если сервис для мониторинга на основном хосте запустится прежде, чем бекап-хост признает его мёртвым.  
Производим `docker-compose up -d` на обоих хостах, находясь в соответствующих директориях (./host и ./backup-host), с не слишком большим интервалом.  
...
После отработки скрипта, когда основной сервис восстановит свою доступность, а все нужные данные с созданной ВМ будут перенесены - просто удалите `LOCK_STATE` в директории `./shared`, созданная ВМ удалится, и мониторинг продолжится в штатном режиме, готовый вновь спасти сервис от падения.  

**На всякий случай**, я продублирую: При удалении `LOCK_STATE` запустится процесс удаления созданной ВМ в яндекс облаке. Не удаляйте файл до того, как перенесёте с ВМ всё что вам нужно.  

## Структура проекта
Как и говорилось, он состоит из двух основных компонентов - 
- `./host` - компонент для работы основного сервиса. Он посылает бекапы на указанный адрес и готовится стать жертвой инцидента. В себе он содержит сервисы:
  - `sync` - контейнер, занимающийся непосредственной отправкой бекапов через `rsync`
  - `app` - контейнер с веб-сервером на nginx, выступающий в роли примера приложения. К нему монтируется директория `data`, являющаяся примером ключевых данных приложения.
- `./backup-host` - компонент для работы бекап-сервера, он-же сервер-последнего-шанса. Он принимает и хранит бекап, опрашивает основной хост о его состоянии и готовится отправить всё в облако при инциденте. Состоит из:
  - `terraform` - контейнер на базе `hashicorp/terraform`. Он отвечает за облачные ресурсы - занимается созданием, настройкой и удалением. Управляется исключительно через состояние `LOCK_STATE` файла, а конфигурируется через переменные окружения в docker-compose.yml.
  - `tunnel` - контейнер двойного назначения, выступает в качестве ssh-приёмника с минимальными правами, а также проверяет доступность сервиса. 
  - `shared` - Директория, выступающая в качестве файлового шлюза между контейнерами. 